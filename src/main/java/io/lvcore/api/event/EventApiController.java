...
import io.lvcore.data.EventDBService;
...
public class EventApiController implements EventApi {
    ...
    public ResponseEntity<Event> createEvent(
        ...
        event.setId(BaseUtils.geneerateId());
        eventDBService.createEvent(event);
    ...
    public ResponseEntity<Event> getEventById(
        ...
        Event event = eventDBService.getEventById(eventId);
        if(event != null) return new ResponseEntity<Event>(event, HttpStatus.OK);
        else throw new NotFoundException(404, "Event not found");
    }
